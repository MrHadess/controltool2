...
public class PackageScan {
     ...
     * 通过包名获取包内所有类
     */
    public static List<Class<?>> scanPackages(List<String> packages){
        List<Class<?>> classList = new ArrayList<Class<?>>();
        for (String item:packages){
            if(item.isEmpty()) continue;
            classList.addAll(getClasses(item));
        }
        return classList;
    }

    @Deprecated
    public static List<Class<?>> nowScanPackage(String s){
        List<Class<?>> classList = new ArrayList<Class<?>>();
...
//        List<Class<?>> classList = getClasses(s);
        ...
        try {
            ...
            while (dirs.hasMoreElements()) {
                ...
                if ("file".equals(protocol)) {
                    ...
                    classes.addAll(loadJarPackageToClass(url,packageDirName,packageName,recursive));
                }
            }
        } catch (IOException e) {
    ...
    private static List<Class<?>> loadJarPackageToClass(URL url,String packageDirName,String packageName,boolean recursive) throws IOException {
        List<Class<?>> classes = new ArrayList<Class<?>>();
        // 如果是jar包文件
        // 定义一个JarFile
        JarFile jar;
